...
import { createGroupSchema, updateGroupSchema, addMemberSchema, updateMemberSchema, transferAdminSchema } from '../validators/groups.validation.js';
...
export const addMember = async (req, res) => {
  try {
    ...
    const member = await groupsService.addMember(req.params.id, validatedData.userId, validatedData.joinedAt);
...
export const updateMember = async (req, res) => {
  try {
    ...
    const member = await groupsService.updateMember(req.params.id, userId, validatedData);
    res.json({ member });
  } catch (error) {
    if (error.name === 'ZodError') {
      const message = error.errors?.[0]?.message || error.issues?.[0]?.message || 'Validation error';
